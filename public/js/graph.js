...
var graphHeight = 600;
var ease = 400;
var logs = logs = data[0].logs.sort(sortDesc), pile;
...
function stretchCanvas(type) {
	...
	width = w()
	canvases[type].height = graphHeight
	$(canvases[type])
		.css({ height: graphHeight })
		.attr('height', graphHeight)
		.attr('resize', false).attr('id',type)
		.appendTo($('#purchase-college .easel'))
...
function graphPoints(logs, type) {
	...
	for(var i = 0; i < logs.length; i++) {
		...
		var y = graphHeight-parseInt(log[type])*yFactor-15
		var data = {
			date: humanDate,
			id: id,
			index: i,
		...
		$(popup)
			...
			.attr('data-type', type)
			.addClass(type)
		$(popup).find('.date .data').html(humanDate)
		$(popup).find('.value .title').html(type)
		$(popup).find('.value .data').html(log[type]+' lbs.')
...
function fillGraphWithSymbols(line, type) {
	...
	for(var i = 0; i < limits.length; i++) {
		var limit = limits[i];
		var size = 25;
		for(var ii = 0; ii < graphHeight-limit.y; ii += 20) {
